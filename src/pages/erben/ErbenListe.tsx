...
import { ERBEN_STATUS } from '../../data/statusNetworks'
...
export function ErbenListe() {
  const { state, dispatch } = useApp()
  const { advanceErbenMandant } = useAdvanceStatus()
  ...
  }, [state.erbenMandanten, fStatus, fPrio, fVerantwortlich, fOverdue])
    ...
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{filtered.length} von {state.erbenMandanten.length} Mandanten</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3">
        <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className={`${inputClass} w-auto`}>
          ...
          {Object.values(ERBEN_STATUS).map((s) => (
      ...
      {view === 'liste' ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50 text-left text-xs font-medium uppercase tracking-wide text-slate-400">
                <th className="px-4 py-3">Mandant/Familie</th>
            ...
            <tbody>
              {filtered.map((k) => {
                ...
                const node = ERBEN_STATUS[k.status]
                return (
                  <tr
                    key={k.id}
                    onClick={() => navigate(`/erben/mandanten/${k.id}`)}
        ...
        <StatusBoard
          bereich="erben"
          basePath="/erben/mandanten"
          onAdvance={(id) => advanceErbenMandant(id)}
      ...
      <NeuerMandantModal open={addOpen} onClose={() => setAddOpen(false)} onCreate={(k) => dispatch({ type: 'ADD_ERBEN_MANDANT', kunde: k })} />
    </div>
  )
}
...
function NeuerMandantModal({ open, onClose, onCreate }: { open: boolean; onClose: () => void; onCreate: (k: Kunde) => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Neuer Mandant">
      <Field label="Mandant/Familie"><input className={inputClass} value={firma} onChange={(e) => setFirma(e.target.value)} /></Field>
      ...
      <Field label="Startstatus">
        <select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}>
          {Object.values(ERBEN_STATUS).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
        </select>
      </Field>
