...
export function BetriebsformenDetail() {
  ...
  if (!mandant) {
    return (
      <div>
        <button onClick={() => navigate('/betriebsformen')} className="mb-4 flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
      ...
      <Card className="mb-5 p-5">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
          ...
          <select
            ...
            onChange={(e) => dispatch({ type: 'SET_BETRIEBSFORMEN_PRIORITAET', id: mandant.id, prioritaet: e.target.value as Prioritaet })}
        ...
        <div className="flex flex-wrap items-center gap-6 border-t border-slate-100 pt-4">
          <div>
            ...
            <StatusWidget
              ...
              onChange={(next) => dispatch({ type: 'SET_BETRIEBSFORMEN_STATUS', id: mandant.id, status: next })}
            />
          </div>
          <div>
