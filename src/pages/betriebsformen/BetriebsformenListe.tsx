...
import { BETRIEBSFORMEN_STATUS } from '../../data/statusNetworks'
...
export function BetriebsformenListe() {
  const { state, dispatch } = useApp()
  const { advanceBetriebsformenMandant } = useAdvanceStatus()
  ...
  const filtered = useMemo(() => {
    return state.betriebsformenMandanten
  ...
  }, [state.betriebsformenMandanten, fStatus, fPrio, fVerantwortlich, fOverdue])
    ...
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{filtered.length} von {state.betriebsformenMandanten.length} Mandanten</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3">
        <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className={`${inputClass} w-auto`}>
          ...
          {Object.values(BETRIEBSFORMEN_STATUS).map((s) => (
      ...
      {view === 'liste' ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((k) => {
                ...
                const node = BETRIEBSFORMEN_STATUS[k.status]
                return (
                  <tr
                    key={k.id}
                    onClick={() => navigate(`/betriebsformen/mandanten/${k.id}`)}
                    ...
                    <td className="px-4 py-3 text-right">
                      {node.next.length > 0 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            advanceBetriebsformenMandant(k.id)
                          }}
        ...
        <StatusBoard
          ...
          basePath="/betriebsformen/mandanten"
          onAdvance={(id) => advanceBetriebsformenMandant(id)}
      ...
      <NeuerMandantModal open={addOpen} onClose={() => setAddOpen(false)} onCreate={(k) => dispatch({ type: 'ADD_BETRIEBSFORMEN_MANDANT', kunde: k })} />
    </div>
  )
}
    ...
    <Modal open={open} onClose={onClose} title="Neuer Mandant">
      ...
      <Field label="Startstatus">
        <select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}>
          {Object.values(BETRIEBSFORMEN_STATUS).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
        </select>
      </Field>
