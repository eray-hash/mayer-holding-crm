...
import { BERATUNG_STATUS } from '../../data/statusNetworks'
...
export function KundenListe() {
  ...
  const { advanceKunde } = useAdvanceStatus()
  ...
  }, [state.kunden, fStatus, fPrio, fVerantwortlich, fOverdue])

  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Kunden</h1>
          <p className="text-sm text-slate-500">{filtered.length} von {state.kunden.length} Kunden</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3">
        <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className={`${inputClass} w-auto`}>
          ...
          {Object.values(BERATUNG_STATUS).map((s) => (
      ...
      {view === 'liste' ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((k) => {
                ...
                const node = BERATUNG_STATUS[k.status]
                return (
                  <tr
                    key={k.id}
                    onClick={() => navigate(`/beratung/kunden/${k.id}`)}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-slate-400">Keine Kunden gefunden.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <StatusBoard
          bereich="beratung"
          basePath="/beratung/kunden"
          onAdvance={(id) => advanceKunde(id)}
      ...
      <NeuerKundeModal open={addOpen} onClose={() => setAddOpen(false)} onCreate={(k) => dispatch({ type: 'ADD_KUNDE', kunde: k })} />
    </div>
  )
}

function NeuerKundeModal({ open, onClose, onCreate }: { open: boolean; onClose: () => void; onCreate: (k: Kunde) => void }) {
  ...
  function submit() {
    ...
    onCreate({
      ...
      activities: [{ id: uid('act'), date: today(), text: 'Kunde angelegt', user: verantwortlich }],
    })
  ...
  return (
    <Modal open={open} onClose={onClose} title="Neuer Kunde">
      <Field label="Firma"><input className={inputClass} value={firma} onChange={(e) => setFirma(e.target.value)} /></Field>
      ...
      <Field label="Startstatus">
        <select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}>
          {Object.values(BERATUNG_STATUS).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
        </select>
      </Field>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <PrimaryButton onClick={submit}>Kunde anlegen</PrimaryButton>
      </div>
    </Modal>
  )
}
