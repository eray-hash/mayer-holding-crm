...
import { BETRIEBSUEBERGABE_STATUS } from '../../data/statusNetworks'
...
export function BetriebsuebergabeListe() {
  const { state, dispatch } = useApp()
  const { advanceBetriebsuebergabeMandant } = useAdvanceStatus()
  ...
  const filtered = useMemo(() => {
    return state.betriebsuebergabeMandanten
  ...
  }, [state.betriebsuebergabeMandanten, fStatus, fPrio, fVerantwortlich, fOverdue])
    ...
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{filtered.length} von {state.betriebsuebergabeMandanten.length} Mandanten</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3">
        <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className={`${inputClass} w-auto`}>
          ...
          {Object.values(BETRIEBSUEBERGABE_STATUS).map((s) => (
      ...
      {view === 'liste' ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50 text-left text-xs font-medium uppercase tracking-wide text-slate-400">
                <th className="px-4 py-3">Unternehmen/Mandant</th>
            ...
            <tbody>
              {filtered.map((k) => {
                ...
                const node = BETRIEBSUEBERGABE_STATUS[k.status]
                return (
                  <tr
                    key={k.id}
                    onClick={() => navigate(`/betriebsuebergabe/mandanten/${k.id}`)}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-500">{k.ansprechpartner}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {node.next.length > 0 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            advanceBetriebsuebergabeMandant(k.id)
                          }}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-slate-400">Keine Mandanten gefunden.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <StatusBoard
          ...
          basePath="/betriebsuebergabe/mandanten"
          onAdvance={(id) => advanceBetriebsuebergabeMandant(id)}
          items={filtered.map((k) => ({
            id: k.id,
            title: k.firma,
            subtitle: k.ansprechpartner,
      ...
      <NeuerMandantModal open={addOpen} onClose={() => setAddOpen(false)} onCreate={(k) => dispatch({ type: 'ADD_BETRIEBSUEBERGABE_MANDANT', kunde: k })} />
    </div>
  )
}

function NeuerMandantModal({ open, onClose, onCreate }: { open: boolean; onClose: () => void; onCreate: (k: Kunde) => void }) {
  const [firma, setFirma] = useState('')
  const [ansprechpartner, setAnsprechpartner] = useState('')
  const [email, setEmail] = useState('')
  const [telefon, setTelefon] = useState('')
  const [prioritaet, setPrioritaet] = useState<Prioritaet>('Mittel')
  const [status, setStatus] = useState('erstkontakt')
  ...
  function submit() {
    ...
    onCreate({
      ...
      activities: [{ id: uid('act'), date: today(), text: 'Mandant angelegt', user: verantwortlich }],
    })
    setFirma(''); setAnsprechpartner(''); setEmail(''); setTelefon('')
    onClose()
  }

  return (
    <Modal open={open} onClose={onClose} title="Neuer Mandant">
      <Field label="Unternehmen/Mandant"><input className={inputClass} value={firma} onChange={(e) => setFirma(e.target.value)} /></Field>
      ...
      <Field label="Startstatus">
        <select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}>
          {Object.values(BETRIEBSUEBERGABE_STATUS).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
        </select>
      </Field>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <PrimaryButton onClick={submit}>Mandant anlegen</PrimaryButton>
      </div>
    </Modal>
  )
}
