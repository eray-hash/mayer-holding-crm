...
import { Users, FileText, ChevronDown, ChevronLeft, ChevronRight, GraduationCap, Sun, LayoutGrid } from 'lucide-react'
...
export function SidebarAkademie({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <nav className="flex h-full flex-col justify-between py-4">
      <div className="space-y-1 px-2">
        ...
        <SideLink
          to="/akademie/projekte"
          icon={<LayoutGrid size={17} />}
          label="Projekte"
          collapsed={collapsed}
          active={isActive('/akademie/projekte')}
        />
        <SideLink
          to="/akademie"
          icon={<Users size={17} />}
          label="Teilnehmer"
          collapsed={collapsed}
          active={isActive('/akademie') && !location.pathname.includes('vorlagen') && !location.pathname.includes('projekte')}
        />
          ...
          {!collapsed && vorlagenOpen && (
            <div className="ml-6 mt-0.5 space-y-0.5 border-l border-slate-100 pl-3">
              {VORLAGEN_LINKS.map((v) => (
                <Link
                  key={v.kind}
                  to={`/akademie/vorlagen/${v.kind}`}
                  className={`block rounded-md px-2 py-1.5 text-sm ${
                    isActive(`/akademie/vorlagen/${v.kind}`) ? 'text-accent-700 font-medium' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {v.label}
                </Link>
      ...
      <div className="px-2">
        {!collapsed && (
          <div className="mb-2 flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-400">
            <GraduationCap size={14} /> Akademie
          </div>
        )}
        <button
          onClick={onToggle}
