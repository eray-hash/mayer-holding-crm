...
import { Users, FileText, ChevronDown, ChevronLeft, ChevronRight, ScrollText, Sun, LayoutGrid } from 'lucide-react'
...
export function SidebarErben({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <nav className="flex h-full flex-col justify-between py-4">
      <div className="space-y-1 px-2">
        ...
        <SideLink to="/erben/projekte" icon={<LayoutGrid size={17} />} label="Projekte" collapsed={collapsed} active={isActive('/erben/projekte')} />
        <SideLink
          to="/erben"
          icon={<Users size={17} />}
          label="Mandanten"
          collapsed={collapsed}
          active={isActive('/erben') && !location.pathname.includes('vorlagen') && !location.pathname.includes('projekte')}
        />
          ...
          {!collapsed && vorlagenOpen && (
            <div className="ml-6 mt-0.5 space-y-0.5 border-l border-slate-100 pl-3">
              {VORLAGEN_LINKS.map((v) => (
                <Link
                  key={v.kind}
                  to={`/erben/vorlagen/${v.kind}`}
                  className={`block rounded-md px-2 py-1.5 text-sm ${
                    isActive(`/erben/vorlagen/${v.kind}`) ? 'text-accent-700 font-medium' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {v.label}
                </Link>
