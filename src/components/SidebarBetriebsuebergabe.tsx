...
import { Users, FileText, ChevronDown, ChevronLeft, ChevronRight, Handshake, Sun, LayoutGrid } from 'lucide-react'
...
export function SidebarBetriebsuebergabe({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <nav className="flex h-full flex-col justify-between py-4">
      <div className="space-y-1 px-2">
        ...
        <SideLink
          to="/betriebsuebergabe/projekte"
          icon={<LayoutGrid size={17} />}
          label="Projekte"
          collapsed={collapsed}
          active={isActive('/betriebsuebergabe/projekte')}
        />
        <SideLink
          to="/betriebsuebergabe"
          icon={<Users size={17} />}
          label="Mandanten"
          collapsed={collapsed}
          active={isActive('/betriebsuebergabe') && !location.pathname.includes('vorlagen') && !location.pathname.includes('projekte')}
        />
          ...
          {!collapsed && vorlagenOpen && (
            <div className="ml-6 mt-0.5 space-y-0.5 border-l border-slate-100 pl-3">
              {VORLAGEN_LINKS.map((v) => (
                <Link
                  key={v.kind}
                  to={`/betriebsuebergabe/vorlagen/${v.kind}`}
                  className={`block rounded-md px-2 py-1.5 text-sm ${
                    isActive(`/betriebsuebergabe/vorlagen/${v.kind}`) ? 'text-accent-700 font-medium' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {v.label}
                </Link>
      ...
      <div className="px-2">
        {!collapsed && (
          <div className="mb-2 flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-400">
            <Handshake size={14} /> Betriebsübergabe
          </div>
        )}
        <button
          onClick={onToggle}
