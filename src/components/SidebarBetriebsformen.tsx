...
import { Users, FileText, ChevronDown, ChevronLeft, ChevronRight, Scale, Sun, LayoutGrid } from 'lucide-react'
...
export function SidebarBetriebsformen({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <nav className="flex h-full flex-col justify-between py-4">
      <div className="space-y-1 px-2">
        ...
        <SideLink
          to="/betriebsformen/projekte"
          icon={<LayoutGrid size={17} />}
          label="Projekte"
          collapsed={collapsed}
          active={isActive('/betriebsformen/projekte')}
        />
        <SideLink
          to="/betriebsformen"
          icon={<Users size={17} />}
          label="Mandanten"
          collapsed={collapsed}
          active={isActive('/betriebsformen') && !location.pathname.includes('vorlagen') && !location.pathname.includes('projekte')}
        />
          ...
          {!collapsed && vorlagenOpen && (
            <div className="ml-6 mt-0.5 space-y-0.5 border-l border-slate-100 pl-3">
              {VORLAGEN_LINKS.map((v) => (
                <Link
                  key={v.kind}
                  to={`/betriebsformen/vorlagen/${v.kind}`}
                  className={`block rounded-md px-2 py-1.5 text-sm ${
                    isActive(`/betriebsformen/vorlagen/${v.kind}`) ? 'text-accent-700 font-medium' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {v.label}
                </Link>
